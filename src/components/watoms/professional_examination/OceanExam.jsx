...
const OceanExam = ({ selectedExamTitle, examQuestions, candidate, onAnswersChange, onChangeExam }) => {
    ...
    const formattedTime = `${seconds.toString().padStart(2, "0")} : ${minutes.toString().padStart(2, "0")}`;
            ...
            <div className="w-3/4 border-white border-2 rounded-xl px-6 py-4 flex flex-col justify-between space-y-4">
                {examQuestions.length > 0 && examQuestions[currentIndex] ? (
                    <div className="space-y-4">
                        <h1 className={`text-sm text-end ${timeLeft < 11 ? "text-red-600" : "text-yellow-400"}`}>
