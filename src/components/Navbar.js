...
const Navbar = ({ children, showNavigate = true, img, length="w-[350px]", upload=false }) => {
    ...
    function typeSubtitle() {
      subtitleTimer = setInterval(() => {
        setTypedSubtitle((prev) => {
          const next = prev + subtitle.charAt(subtitleIndex);
          subtitleIndex++;
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: upload ? "none" : `url(${slides[current].image})`,
    ...
    >
      <div className="w-full flex justify-between items-center px-4 py-2">
        ...
        <div
          className={`hidden md:flex items-center gap-4 bg-white py-2 px-2 rounded-full shadow-lg shadow-black/30 transition-all duration-300 ease-in-out overflow-hidden ${
            profileHover ? length : "w-[55px]"
          }`}
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <div className={`${profile} flex justify-evenly items-center`}>{children}</div>
