...
const CustomHorizontalBarChart = ({ data, title }) => {
    return (
        <div className="relative w-full h-fit max-h-54 flex flex-col items-stretch bg-[#2d3347] rounded-2xl py-2 mb-0 overflow-hidden shadow-[0_2px_12px_#0004]" >
            ...
            <div className="min-h-[60px] max-h-[300px] flex flex-col justify-start overflow-y-auto overflow-x-hidden mt-[9px] z-[1]" >
                {data.map((singleData, idx) => (
                    ...
                    >
                        <div className="min-w-[115px] max-w-[120px] text-[15px] text-[#fff] text-start font-black overflow-hidden mr-2 truncate transition-colors duration-200 ease-in-out" >
                            {singleData?.name}
                        </div>
                        <div className="relative flex-1 min-w-0 max-w-[170px] h-[20px] rounded-[18px] overflow-hidden transition-shadow duration-200 ease-in-out mx-2" >
                            <div className={`h-full w-[${Math.min(100, Math.max(0, roundNumber(((singleData?.performance / data[0]?.performance) * 100) || 0)))}%] ${WATOMS_MODERN_COLORS_TW[idx]} rounded-[18px] transition-[width] duration-[700ms] ease-[cubic-bezier(.4,2,.6,1)]`} />
                        </div>
