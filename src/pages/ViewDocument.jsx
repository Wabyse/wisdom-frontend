...
const ViewDocument = () => {
  ...
  return (
    <div className="bg-gray-500 h-screen">
      <Navbar upload={true} length="w-[500px]">
        <ChangeLanguage />
      </Navbar>
      <div className=" bg-slate-600 text-white p-[20px] rounded-[8px] max-w-[600px] w-full m-auto shadow-md mt-2">
        ...
        <div className="flex justify-between gap-[15px]">
          <div className="text-center border-2 border-black m-[2px] flex-1 p-[10px] rounded-[6px] bg-slate-400">
            <label className="text-slate-600 text-[13px] font-extrabold block mb-[3px]">{language ? "Employee's Organization:" : ":الجهة التابع لها الموظف"}</label>
          ...
          {document?.uploader.employee?.teacher ? <div className="text-center border-2 border-black m-[2px] flex-1 p-[10px] rounded-[6px] bg-slate-400">
            <label className="text-slate-600 text-[13px] font-extrabold block mb-[3px]">{language ? "Employee's Department:" : ":القسم التابع له الموظف"}</label>
            <h1 className="text-lg m-0 font-normal">{`${document?.uploader.employee?.teacher?.department.Name}`}</h1>
          </div> : null}
        </div>
        {document?.uploader.employee?.teacher ? <div className="text-center border-2 border-black m-[2px] flex-1 p-[10px] rounded-[6px] bg-slate-400">
          <label className="text-slate-600 text-[13px] font-extrabold block mb-[3px]">{language ? "Employee's Subject:" : ":المادة"}</label>
